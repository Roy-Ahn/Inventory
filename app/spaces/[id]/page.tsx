...
export default function SpaceDetailPage() {
  ...
  const initiateBooking = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-intent', {
        body: { amount: Math.round(parseFloat(totalPrice) * 100), currency: 'usd' }
      });
  ...
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    ...
    try {
      ...
      console.error("Error creating booking after payment:", {
        message: errorMessage,
        error: err,
      });
      setBookingError(`Payment successful but booking creation failed: ${errorMessage}. Please contact support.`);
              ...
              <div className="bg-blue-50 border border-blue-200 p-6 rounded-xl">
                ...
                {!currentUser ? (
                  <div className="text-center">
                    ...
                    <button onClick={() => router.push('/login')} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700">
                      Login to Book
                    </button>
                  </div>
                ) : space.isAvailable ? (
